...
func (mx *Mux) Use(middlewares ...func(http.Handler) http.Handler) {
	if mx.handler != nil {
		panic("bastigo: all middlewares must be defined before routes on a mux")
	}
...
func (mx *Mux) Method(method, pattern string, handler http.Handler) {
	...
	if !ok {
		panic(fmt.Sprintf("bastigo: '%s' http method is not supported.", method))
	}
...
func (mx *Mux) Route(pattern string, fn func(r Router)) Router {
	if fn == nil {
		panic(fmt.Sprintf("bastigo: attempting to Route() a nil subrouter on '%s'", pattern))
	}
	subRouter := NewRouter()
	fn(subRouter)
...
// Mount attaches another http.Handler or bastigo Router as a subrouter along a routing
// path. It's very useful to split up a large API as many independent routers and
// compose them as a single service using Mount. See _examples/.
//
// Note that Mount() simply sets a wildcard along the `pattern` that will continue
// routing at the `handler`, which in most cases is another bastigo.Router. As a result,
// if you define two Mount() routes on the exact same pattern the mount will panic.
func (mx *Mux) Mount(pattern string, handler http.Handler) {
	if handler == nil {
		panic(fmt.Sprintf("bastigo: attempting to Mount() a nil handler on '%s'", pattern))
	}

	// Provide runtime safety for ensuring a pattern isn't mounted on an existing
	// routing pattern.
	if mx.tree.findPattern(pattern+"*") || mx.tree.findPattern(pattern+"/*") {
		panic(fmt.Sprintf("bastigo: attempting to Mount() a handler on an existing path, '%s'", pattern))
	}
...
func (mx *Mux) handle(method methodTyp, pattern string, handler http.Handler) *node {
	if len(pattern) == 0 || pattern[0] != '/' {
		panic(fmt.Sprintf("bastigo: routing pattern must begin with '/' in '%s'", pattern))
	}
...
func (mx *Mux) routeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// Check if method is supported by bastigo
	if rctx.RouteMethod == "" {
